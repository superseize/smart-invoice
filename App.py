...
# tkinter/PIL splash-screen imports are DESKTOP-ONLY (used by show_splash() when
# running as a local .exe). On a server (Render/Linux/no display) these can be
# missing or fail to initialize, so they must never crash a normal web boot.
try:
...
# ============================================================
# DEVELOPER-FIXED EMAIL SENDER (used to send OTP codes).
# Fill these ONCE here — end users never see or configure SMTP.
# Gmail: turn on 2-Step Verification, then create an App Password at
# myaccount.google.com/apppasswords (16-digit code, not your normal password).
# ============================================================
DEFAULT_SMTP_HOST = "smtp.gmail.com"
DEFAULT_SMTP_PORT = "587"
DEFAULT_SMTP_USER = "ishtiaqahmadmagray@gmail.com"          # tumhara fixed Gmail
DEFAULT_SMTP_APP_PASSWORD = ""  # the 16-digit Gmail App Password
DEFAULT_SMTP_FROM_NAME = "Smart Invoice"
DEFAULT_SMTP_USE_TLS = "1"
...
.auth-box{background:#ffffff;padding:40px;border-radius:20px;box-shadow:0 20px 50px rgba(0,0,0,.35);width:400px;max-width:100%;text-align:center}
...
.auth-box input{width:100%;padding:14px 16px;margin:8px 0;border:1.5px solid #dde8e5;border-radius:12px;font-size:16px;box-sizing:border-box;background:#f7faf9;transition:border-color .15s}
...
.masked-email{display:inline-block;background:#ecfdf5;color:#065f46;font-weight:700;padding:4px 12px;border-radius:20px;font-size:13px;margin:10px 0;word-break:break-all}
...
def login():
    ...
    """
    return render_template_string(html, email_hint=f"Signed in as {mask_email(admin_email)}")
...
def setup_account():
    ...
    if request.method == "POST":
        ...
        if not email or "@" not in email:
            ...
            flash("Password must be 4–10 characters")
    ...
    <body>
        <div class="box">
            ...
            <form method="post">
                ...
                <input type="password" name="password" placeholder="Create Password (4-10 characters)" minlength="4" maxlength="10" required>
                <input type="password" name="confirm" placeholder="Confirm Password" maxlength="10" required>
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        if not email or "@" not in email:
            ...
            if admin_email and email == admin_email.lower():
                ...
                if cooldown <= 0:
                    ok, msg = generate_and_send_otp(email, purpose="reset")
                    if not ok:
                        flash(f"⚠️ {msg}")
                    else:
    ...
    <body>
        <div class="auth-box">
            ...
            <form method="post">
                ...
                {% if masked %}
                <div class="cooldown" style="margin:6px 0 0">Registered email: <b style="color:#0d9488">{{ masked }}</b></div>
                {% endif %}
                <button type="submit" class="auth-btn" style="margin-top:14px">Send Verification Code</button>
            </form>
    ...
    """
    return render_template_string(html, masked=mask_email(admin_email) if admin_email else "")
def verify_otp():
    ...
    if request.method == "POST":
        if request.form.get("resend"):
            ...
            if cooldown > 0:
                ...
                ok, msg = generate_and_send_otp(email, purpose="reset")
                flash("A new verification code has been sent." if ok else f"⚠️ {msg}")
        else:
...
def set_new_password():
    ...
    <body>
        <div class="auth-box">
            ...
            {% endwith %}
            <form method="post">
                <input type="password" name="new_password" placeholder="New password (4-10 characters)" minlength="4" maxlength="10" required autofocus>
                <input type="password" name="confirm" placeholder="Confirm new password" maxlength="10" required>
...
def _smtp_config():
    """Per-install Settings (if the user filled them) win; otherwise fall back
    to the developer-fixed DEFAULT_SMTP_* constants defined near the top of this file,
    so end users never have to configure anything."""
    host = get_setting("smtp_host", "").strip() or DEFAULT_SMTP_HOST
    port = get_setting("smtp_port", "").strip() or DEFAULT_SMTP_PORT
    user = get_setting("smtp_user", "").strip() or DEFAULT_SMTP_USER
    app_pw = get_setting("smtp_app_password", "").strip() or DEFAULT_SMTP_APP_PASSWORD
    from_name = get_setting("smtp_from_name", "").strip() or DEFAULT_SMTP_FROM_NAME
    use_tls = (get_setting("smtp_use_tls", "").strip() or DEFAULT_SMTP_USE_TLS) == "1"
    return host, port, user, app_pw, from_name, use_tls


def send_email(to_email: str, subject: str, body_html: str):
    """Sends email via SMTP. Uses per-install Settings if filled, otherwise the
    developer-fixed DEFAULT_SMTP_* constants. Returns (success, message).
    Never logs/prints the full email body (may contain OTP)."""
    host, port, user, app_pw, from_name, use_tls = _smtp_config()
...
def send_email_with_attachment(to_email: str, subject: str, body_html: str, file_path, file_display_name: str = None):
    """Same SMTP resolution as send_email, but attaches a single local file (e.g. a backup .zip).
    ...
    if size_mb > 24:
        return False, f"Backup is {size_mb:.1f}MB — too large to email (limit ~24MB). Use the local download instead."
    ...
    try:
        ...
        part.add_header("Content-Disposition", f'attachment; filename="{file_display_name or file_path.name}"')
        msg.attach(part)
        ...
        print("❌ send_email_with_attachment error:", type(e).__name__)
        return False, "Could not send backup email right now. Please try again shortly."
...
def home():
    ...
    # Today's Sale + Today's Expense — parsed comparison (not raw string match),
    # so it works no matter which date format each record was saved with.
    today_d = now.date()

    def _parse_any_date(d):
        d = (d or "").strip()
        if not d:
            return None
        for fmt in ("%d-%m-%Y", "%d-%m-%y", "%Y-%m-%d", "%m/%d/%Y", "%d/%m/%Y"):
            try:
                ...
                continue
        # last resort: date might have a time component appended
        try:
            return datetime.datetime.fromisoformat(d[:10]).date()
        except Exception:
            return None
...
function loadHist(){
  ...
  fetch(`/api/history?name=${encodeURIComponent(n)}&address=${encodeURIComponent(a)}`).then(r=>r.json()).then(d=>{
    ...
    if(d.customer_type){
      ...
      if(ctypeSel){ ctypeSel.value = d.customer_type; }
    }
    // Re-run setInfo on every already-selected product row now that customer
    // name/address are known — this is what fills "Show Last Bill Prices",
    // which previously stayed blank if the product was picked before typing the customer.
    if(typeof updatePriceType==='function') updatePriceType();
    if(d.salesman){
...
def open_pdf_path(y, m, fn):
    ...
    if not base.exists():
        try:
            ...
            flash(f"Folder not found and could not create folder: {e}")
            return redirect(url_for("reports"))
    fp = base / fn
    if not fp.exists():
...
def settings():
    if request.method == "POST":
        ...
        if "smtp_host" in request.form:
            ...
            set_setting("smtp_port", request.form.get("smtp_port", "587").strip() or "587")
            set_setting("smtp_user", request.form.get("smtp_user", "").strip())
            new_smtp_pw = request.form.get("smtp_app_password", "").strip()
            if new_smtp_pw:  # only overwrite if a new one was typed (field shown blank for security)
                set_setting("smtp_app_password", new_smtp_pw)
            set_setting("smtp_from_name", request.form.get("smtp_from_name", "").strip() or "Smart Invoice")
        ...
        if new_password:
            if 4 <= len(new_password.strip()) <= 10:
                if security_question and security_answer:
                    set_setting("admin_password_hash", generate_password_hash(new_password.strip()))
                    set_setting("security_question", security_question.strip())
                    set_setting("security_answer", security_answer.strip().lower())
                    try:
                        set_setting("session_version", str(int(get_setting("session_version", "1")) + 1))
                ...
                flash("New password must be 4–10 characters long")

        # ==================== LOGIN EMAIL CHANGE ====================
        new_login_email = (request.form.get("new_login_email") or "").strip().lower()
        current_password_confirm = request.form.get("current_password_confirm") or ""
        if new_login_email:
            admin_password_hash = get_setting("admin_password_hash", "")
            if "@" not in new_login_email:
                flash("Please enter a valid email address")
            elif not current_password_confirm or not check_password_hash(admin_password_hash, current_password_confirm):
                flash("Current password is incorrect — login email not changed")
            else:
                set_setting("admin_email", new_login_email)
                flash(f"Login email updated to {new_login_email}")
...
@media (max-width:700px){
  ...
  .set-card-header{padding:14px 18px;font-size:15px}
...
<div class="set-card">
  ...
  <form method="post">
    ...
    <input name="new_password" type="password" placeholder="4-10 characters" minlength="4" maxlength="10">
...
<div class="set-card">
  <div class="set-card-header" style="background:linear-gradient(135deg,#4338ca,#312e81)">📧 Update Login Email</div>
  <div class="set-card-body">
  <p style="color:var(--text-muted); margin-bottom:6px;">This is the email used to log in and to receive password-reset OTP codes.</p>
  <form method="post">
    <label>New Login Email</label>
    <input name="new_login_email" type="email" placeholder="new-email@example.com">
    <label>Current Password (to confirm it's you)</label>
    <input name="current_password_confirm" type="password" placeholder="Enter current password">
    <div style="margin-top:22px;text-align:right;">
      <button class="btn btn-primary btn-lg">Update Login Email</button>
    </div>
  </form>
  </div>
</div>
...
<div class="set-card">
  <div class="set-card-header" style="background:linear-gradient(135deg,#dc2626,#991b1b)">✉️ Email (SMTP) Settings — for OTP Codes</div>
  <div class="set-card-body">
  <p style="color:var(--text-muted); margin-bottom:6px;">
    Gmail ke liye: <b>smtp.gmail.com</b>, port <b>587</b>, apna Gmail address, aur "App Password" (normal Gmail password nahi —
    2-Step Verification on karo, phir myaccount.google.com/apppasswords se 16-digit app password banao).
  </p>
  <form method="post">
    <div class="set-grid">
      <div>
        <label>SMTP Host</label>
        <input name="smtp_host" value="{{smtp_host}}" placeholder="smtp.gmail.com">
      </div>
      <div>
        <label>SMTP Port</label>
        <input name="smtp_port" value="{{smtp_port}}" placeholder="587">
      </div>
      <div>
        <label>Sender Email (Gmail Address)</label>
        <input name="smtp_user" type="email" value="{{smtp_user}}" placeholder="you@gmail.com">
      </div>
      <div>
        <label>App Password {% if smtp_configured %}(already set — leave blank to keep){% endif %}</label>
        <input name="smtp_app_password" type="password" placeholder="{% if smtp_configured %}•••••••••••••• (leave blank to keep){% else %}16-digit app password{% endif %}">
      </div>
      <div>
        <label>From Name</label>
        <input name="smtp_from_name" value="{{smtp_from_name}}" placeholder="Smart Invoice">
      </div>
    </div>
    <label class="set-check" style="margin-top:14px"><input type="checkbox" name="smtp_use_tls" {% if smtp_use_tls=='1' %}checked{% endif %} style="width:auto"> Use TLS (recommended — keep checked for Gmail)</label>
    <div style="margin-top:22px;text-align:right;">
      <button class="btn btn-danger btn-lg">💾 Save Email Settings</button>
    </div>
  </form>
  </div>
</div>
""" + TPL_F
    ...
    return render_template_string(html,
        ...
        smtp_from_name=get_setting("smtp_from_name", "Smart Invoice"),
        smtp_use_tls=get_setting("smtp_use_tls", "1"),
        smtp_configured=bool(get_setting("smtp_app_password", "").strip())
    )
# ================== سیلز ریکارڈ ==================
...
def backup_restore():
    ...
    if request.method == "POST":
        ...
        # ============ CLOUD BACKUP (EMAIL THE ZIP TO ADMIN) ============
        elif action == "cloud_backup":
            admin_email = get_setting("admin_email", "").strip()
            if not admin_email:
                flash("No login email set up yet — set it in Settings first.")
                return redirect(url_for("backup_restore"))
            try:
                timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                backup_filename = f"SmartInvoice_CloudBackup_{timestamp}.zip"
                backup_path = backup_dir / backup_filename
                ...
                ok, msg = send_email_with_attachment(
                    admin_email,
                    f"Smart Invoice Backup — {timestamp}",
                    f"<p>Automatic backup attached, taken on {timestamp}.</p>"
                    f"<p>Keep this email safe — it contains your full business data. "
                    f"To restore, use the 'Restore from Backup' option in Settings → Backup.</p>",
                    backup_path,
                    file_display_name=backup_filename
                )
                try:
                    backup_path.unlink()  # don't keep a second copy on disk after emailing
                except Exception:
                    pass

                if ok:
                    set_setting("last_cloud_backup_at", datetime.datetime.now().isoformat())
                    flash(f"☁️ Backup emailed to {mask_email(admin_email)}")
                else:
                    flash(f"⚠️ {msg}")
            except Exception as e:
                flash(f"Error creating cloud backup: {str(e)}")
            return redirect(url_for("backup_restore"))
...
<style>
@media (max-width:900px){ .backup-grid{grid-template-columns:1fr !important} }
</style>
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:30px;margin:30px 0;" class="backup-grid">
  <!-- Create Backup -->
  ...
  <!-- Cloud Backup (Email) -->
  <div class="card" style="border-left:6px solid #0d9488; text-align:center;">
    <h3 style="color:#0d9488;">☁️ Backup to Cloud</h3>
    <p>Poori backup email pe bhej degi ({{ admin_email_masked }}). Internet zaroori hai.</p>
    {% if last_cloud_backup %}<p style="font-size:12px;color:#888;">Last cloud backup: {{ last_cloud_backup }}</p>{% endif %}
    <form method="post">
      <input type="hidden" name="action" value="cloud_backup">
      <button type="submit" class="btn-3d" style="background:#0d9488; {button_3d_style}"
              onmouseover="this.style.{button_3d_hover}"
              onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 8px 0 rgb(0,0,0,0.3), 0 12px 20px rgba(0,0,0,0.4)';">
        ☁️ Backup Now (Email)
      </button>
...
""" + TPL_F
    ...
    return render_template_string(
        html, backups=backups, project=get_setting("project_name"), message=message,
        admin_email_masked=mask_email(get_setting("admin_email", "")),
        last_cloud_backup=get_setting("last_cloud_backup_at", "")
    )
...
if __name__ == "__main__":
    ...
    if DESKTOP_UI_AVAILABLE:
        # ---------- DESKTOP MODE (local .exe with its own window) ----------
        import webview
        show_splash(company)

        def run_flask():
            app.run(host="127.0.0.1", port=20497, debug=False, use_reloader=False)
        ...
        # ---------- SERVER MODE (Render / any Linux host, no GUI) ----------
        # Normally gunicorn imports `app` directly and this branch never runs,
        # but this keeps `python App.py` working too as a plain web server.
